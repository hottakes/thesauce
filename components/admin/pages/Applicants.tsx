...
export const AdminApplicants = () => {
  ...
  const handleSelectAll = () => {
    if (selectedIds.length === applicantsData?.data.length) {
      ...
      setSelectedIds(applicantsData?.data.map((a: Applicant) => a.id) || []);
    }
  };
  ...
  const exportCSV = () => {
    ...
    const rows = applicantsData.data.map((a: Applicant) => [
      a.instagram_handle,
      a.email || '',
      a.school,
      a.status,
            ...
            <table className="w-full">
              ...
              <tbody>
                {isLoading ? (
                  ...
                  applicantsData?.data.map((applicant: Applicant) => (
                    <tr
                      key={applicant.id}
